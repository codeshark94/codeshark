...
Before every non-restricted local-console or Telegram task, Project Router chooses one scope from the request, current project context, bounded project memory, assistant assets, and recent delivered files. The source of truth for a project is a physical direct-child directory of the configured workspace: every visible directory is registered as a project immediately except the transport-only `inbox` and `deliverables` folders. Router may continue one of those existing projects, connect a clear follow-up to one, create a new directory only when the user explicitly requests a project, or keep the work in `General`. It never invents a project for an ordinary request. An explicit unambiguous project path or name wins deterministically; `/project NAME` remains the manual override. Before the first task in a real project, Codeshark creates `<project>/PROJECT_SSOT.md` when absent. The owner-managed brief stays intact, while a separate Codeshark-managed section records explicit project goals, constraints, decisions, status, and deliverables. Each project has its own temporary Codex session, long-term memories, and assistant assets. A project session is created on its first task and then preserved independently per conversation. `/clear_temp` (or `/new`) deletes only the current project's temporary session; it never deletes long-term records. A safe private-chat follow-up sent during an active task steers that live Codex turn only when it belongs to the same active project. Scheduled work retains the project selected when it was created and runs ephemerally. The configured number of independent tasks can run concurrently, while tasks that share a persistent chat session remain serialized. The queue survives service restarts.
...
Automatic learning is evidence-bound. During a project task, Triage also identifies explicit major project details from the current administrator request and stores them in the project's long-term memory and `PROJECT_SSOT.md`; it never infers or rewrites a user-maintained brief. An exact, non-secret statement from the current administrator request may be applied immediately. Inferred or transformed candidates remain pending for review. One-off details, speculation, credentials, and unnecessary sensitive data are excluded.
...
- Non-administrator group requests are ephemeral, MCP-disabled, and confined to a separate sandbox; they may perform ordinary network research, create or modify only sandbox files, and attach only the current request's safe result files to that same group when group file delivery is enabled. The newest 12 messages and successful Codeshark exchanges Telegram delivers from one group form that group's shared, 30-day context; they never enter private chats or other groups.
...
- [x] Broad primary-validator-reconciliation workflow for substantive work, with manuscript review as one specialization
